	...
	function generate ($data, $settings = array ()) {
		...
		foreach ($data as $i => $result) {
			...
			if ($element) {
                ...
                /**
                 * Modify by me for sidebar menu
                 */
                if ($hasChildren) {
                    $content = '<strong><strong><strong>' . $result[$model][$alias] . '</strong></strong></strong>';
                } else {
                    $content = $this->Html->link('<strong><strong><strong>' . $result[$model][$alias] . '</strong></strong></strong>',
                                $result[$model]['url'], array('escape' => false, 'class' => 'sb_item'), false, false);
                }
			}
			if (!$content) {
				continue;
